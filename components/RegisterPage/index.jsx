...
function RegisterPage() {
  ...
  async function submitHandler(event) {
    ...
    try {
      const result = await registerUser(enteredFirstName, enteredLastName, enteredUsername, enteredPassword);
      if(result.success === '1'){
      ...
      <div className="bg-sky-900 w-4/5 mx-auto text-slate-200 rounded-xl py-10">
        ...
        <form className="mx-auto w-fit" onSubmit={submitHandler}>
          <div className="grid grid-cols-4 grid-rows-4 gap-2 mt-4">
            ...
            <input type="text" ref={firstNameInputRef} className="h-10 ml-12 rounded-md px-2 text-cyan-950 col-span-3 min-w-full" />
            <label>Last Name</label>
            <input type="text" ref={lastNameInputRef} className="h-10 ml-12 rounded-md px-2 text-cyan-950 col-span-3 min-w-full" />
            ...
            <button type="submit" className="bg-sky-950 h-10 w-44 rounded-md col-span-4 justify-self-center mt-4">Register</button>
          </div>
        </form>
      </div>
    </div>
